...
But fiber particle is exerted fiber-fiber spring force, pressure gradient (for incompressible condition) and viscous by fluid and wall
...
fiber_consist = (int *)malloc(n_fiber * l_f * a_f * sizeof(int));
...
for (int i = 0; i < l_f * a_f * n_fiber ; i++){
  fscanf(fp_input,"%d\n",&tf);
  fiber_consist[i] = tf ;
}
fclose(fp_input);

n_solid = n_fiber * l_f * a_f;
// calculate constant ---------------------------------------------------------------------------------------------------------------------
...
while(1){
  ...
  if(aspect_ratio>0) calFiberRestoreForce_thick(l_f,a_f,n_fiber,fiber_consist,Position,Acceleration,parameters,linklist,fiber_l_ave,fiber_theta_ave);
  //move particle
  // if(parameters->Gravity != 0.0){
  ...
  if(aspect_ratio>0){
    if ( iTimeStep % ((int)(parameters->OUTPUT_INTERVAL/10)) == 0 ){
      output_tanphi_thick(l_f,a_f,n_fiber,fiber_consist,Position,parameters,Time,linklist);
